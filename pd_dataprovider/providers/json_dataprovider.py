...
class JSONDataProvider(GenericDataProvider):
    ...
    def __init__(self, paths, keys, verbose = 0, epoch=False, **kwargs):
        super(JSONDataProvider, self).__init__(self.logger, verbose, tz='America/New_York', **kwargs)
        self.paths = paths
        self.keys = keys
        self.epoch = epoch
    ...
    def json_to_df(self, filename: str, symbol_data: SymbolData) -> pd.DataFrame:
        with open(filename) as f:
            ...
            if json_data[symbol_data.symbol]:
                df = pd.DataFrame(json_data[symbol_data.symbol], columns=self.keys)
                df.rename(columns={self.keys[0]: GenericDataProvider.DEFAULT_COL_NAMES[0],
                                   self.keys[1]: GenericDataProvider.DEFAULT_COL_NAMES[1],
                                   self.keys[2]: GenericDataProvider.DEFAULT_COL_NAMES[2],
                                   self.keys[3]: GenericDataProvider.DEFAULT_COL_NAMES[3],
                ...
                if self.epoch:
                    ...
                    df.index = pd.to_datetime(df.index, utc=True).tz_convert(self.tz).tz_localize(None)
                self.logger.info("{}, {:d} rows ({} to {})".format(f.name, len(df), df.index[0], df.index[-1]))
                return df
            else:
                self.logger.warning(f"Could not find '{symbol_data.symbol}' in file '{filename}'")
            return pd.DataFrame()
        ...
        for path in self.paths:
            ...
            if os.path.exists(filename):
                ...
                if not df.empty and 'snapshots' in kwargs and kwargs['snapshots'] and symbol_data.timeframe == 'day':
                    ...
                    self.logger.debug(f"Trying snapshot file: '{snapshot_filename}'")
                    # read snapshots file in the same manner and append to the daily bars (or overwrite if exits)
                    snapshot_df = self.json_to_df(snapshot_filename, symbol_data)
                    if snapshot_df.empty:
                        self.logger.warning(f"Failed to load snapshot file '{snapshot_filename}'")
                    else:
                        if snapshot_df.index.isin(df.index):
                            self.logger.debug(f"Not adding already existing data point for snapshot '{snapshot_filename}'")
                        else:
